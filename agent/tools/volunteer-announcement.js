...
/**
 * Build the create_volunteer_announcement tool. `onDraft` receives the draft's
 * type and core content each time the tool runs, so the caller can remember the
 * most recent draft for follow-up edits. The text returned to the model is unchanged.
 * @param {(draft: { type: string, content: string }) => void} [onDraft]
 */
export function createVolunteerAnnouncementTool(onDraft) {
  return tool(
    ...
    async ({ event_name, date, time, volunteers_needed, location, skills_needed, contact_person }) => {
      ...
      if (onDraft) onDraft({ type: 'volunteer announcement', content: announcement });
